...
public class AbstractsCorpus extends
    ...
    int mMaxSupervisedInstancesPerCategory = 10;

    public AbstractsCorpus(File path) throws IOException {
        File trainDir = new File(path, "train");
        File testDir = new File(path, "test");
        // File unlabelDir = new File(path, "unlabeled");
    ...
    public String toString() {
        ...
        sb.append("TOTALS: #train=" + totalTrain + " #test=" + totalTest
    ...
    private static Map<String, String[]> read(File dir) throws IOException {
        ...
        for (File catDir : dir.listFiles()) {
            ...
            for (File file : catDir.listFiles()) {
                String[] lines = FileLineReader.readLineArray(file, "UTF-8");
    ...
    // Tokenizer tokenizer = EMNaiveBayesClassifier.TOKENIZER_FACTORY
    ...
    private static void visit(Map<String, String[]> catToItems,
            ObjectHandler<Classified<CharSequence>> handler, int maxItems) {
        for (Map.Entry<String, String[]> entry : catToItems.entrySet()) {
            ...
            for (int i = 0; i < maxItems && i < texts.length; ++i) {
                Classified<CharSequence> classifiedText = new Classified<CharSequence>(
                        texts[i], c);
